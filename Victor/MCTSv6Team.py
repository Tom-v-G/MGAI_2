...
class MCTSAgent(CaptureAgent):
    ...
    def get_teammate_indices(self, gameState):
        if self.red: idxs = gameState.getRedTeamIndices()
        else : idxs = gameState.getBlueTeamIndices()
        idx_list = []
        for idx in idxs:
            if idx != self.index:
                idx_list.append(idx)
    ...
    def chooseAction(self, gameState, searchDepth=80, soft_time_limit=0.55, gamma=0.60):
        ...
        if self.defensive: def_idx = self.index
        else : def_idx = self.get_teammate_indices(gameState)[0]
        
        self.calculate_closest_pacman(gameState, def_idx)
        self.choose_target_food(gameState)
    ...
    def calculate_closest_pacman(self, gameState, def_idx):
        if self.red: enemyidxs = gameState.getBlueTeamIndices()
        else: enemyidxs = gameState.getRedTeamIndices()
        ...
        if enemy_pos_list:
            for i in enemy_pos_list:
                dist = self.getMazeDistance(gameState.getAgentPosition(def_idx), gameState.getAgentPosition(i))
                if  dist < smallest_dist:
                    smallest_dist = dist
                    enemy_pacman = i
        MCTSAgent.closest_pacman = enemy_pacman

    def choose_target_food(self, gameState):
        if  MCTSAgent.targetFood is not None:
            if MCTSAgent.delta > 0:
               MCTSAgent.delta -= 1 
               return True
